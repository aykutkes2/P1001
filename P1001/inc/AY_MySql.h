...
class MySqlConnect {
	...
	const char Schema[20] = "devicelist_001";
	const char Table[20] = "gatewaylist_001";
	const char UserList[20] = "user_list_001";
		...
		*/
		...
		if (!qstate)
		{
			...
			if ((row = mysql_fetch_row(res)))///< existing user
			{
				...
				if (_String_Compare(row[USR_Pwd], Pass) == TRUE) {///< PASSWORD CORRECT
					/*
					...
					*/
					...
					if (!qstate)
					{
						...
						if (result == 0) {	
							...
							_String_Add((char *)"Undefined Gateway");		_Break_Add();
							_Ui32_Array_Add(&Unique[0], 3);					_Break_Add();
							_Ui64_Add(Mac);									_Break_Add();
							_Ui32_Add(GATEWAY_UserIDX);						_Break_Add();
							_Ui32_Add(CertificateID);						_Break_Add();
							_Ui32_Add(ConnId);								_Break_Add();
							_Ui64_Array_Add(&_SessionKeyTemp[0], 3);		_Break_Add();
							_Ui32_Add(0);									_Break_Add();
							_Ui32_Add(0);									_Break_Add();
							_Ui32_Add(0);									_Break_Add();
							_Ui32_Add(0);									_Break_Add();
							_Time_Add(_TimeToMysqTime(localtime(&Time)));	_Break_Add();
							_Time_Add(_TimeToMysqTime(localtime(&Time)));	_Break_Add();
...
#ifdef WIN32
					cout << "WRONG PASSWORD !!!  " << row[3] << " -- " << Pass << endl;
#endif
