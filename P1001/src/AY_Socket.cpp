...
int UDP_packet_send(Ui08 idx, udp_headerAll * UDP_header, Ui08 *pBuff, int len) {
	int i;
	udp_headerAll	*pHdr = (udp_headerAll *)&SocketBuff[0];
	//pcap_t *fp = (pcap_t *)pFp;
	i = sizeof(udp_headerAll);
	memcpy(&SocketBuff[0], UDP_header, i);
	memcpy(&SocketBuff[i], pBuff, len);
	i += len;
	...
	pHdr->_ipHeader.crc = ~(uip_ipchksum(&SocketBuff[0])); 				///< header checksum
	pHdr->_udpHeader.crc = ~(uip_udpchksum(&SocketBuff[0]));  			///< checksum

	/* Send down the packet */
	if (pcap_sendpacket(Thrd[idx].pfp, SocketBuff, i) != 0)
	{
