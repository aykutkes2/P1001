...
Device 1 Ghost			192.168.2.148	(4)		(Client 1)
Demo					192.168.2.150	(3)		
*/
#include <stdio.h>
#include <time.h>
#include <windows.h>
	...
	while (1) {
		...
		if (!AY_Demo_Intro) {
			...
			if (!AY_Demo_WaitMACadr) {
				if (AYSCKT_Socket_Init(_SLVS_SCKT, (Ui08 *)&MyMac[0], &MyIP_Address.byte1, 0, 0, AY_SocketRead_CallBack, AY_DemoInitLoop) == 1) {
					AY_Demo_WaitMACadr = 1; 
					j = 0;
				}
			}
